...
// Variant B (compact) and Variant C (portal) are opt-in through the Customizer
// and must never load without an explicit choice: any value the sanitizer does
// not know falls back to Variant A, the theme's default three-row header. The
// allowlist is repeated here rather than trusted from the stored value, so a
// theme_mod written directly to the database cannot name a template part.
// See template-parts/header-*.php.
$probo_header_variant = probo_get( 'header_variant' );

if ( ! in_array( $probo_header_variant, array( 'compact', 'portal' ), true ) ) {
	$probo_header_variant = 'ruim';
}

get_template_part( 'template-parts/header', $probo_header_variant );
?>
