...
<main class="pp-container py-14">
		...
		<header class="mb-8">
			<h1 class="text-4xl font-extrabold tracking-[-0.035em] lg:text-5xl">
				...
				if ( is_search() ) {
					...
					printf( esc_html__( 'Search results for “%s”', 'probo-connect-theme' ), esc_html( get_search_query() ) );
				} elseif ( is_archive() ) {
					the_archive_title();
				} else {
					esc_html_e( 'News', 'probo-connect-theme' );
				}
				?>
			</h1>
	...
	<?php else : ?>
		<h1 class="text-4xl font-extrabold tracking-[-0.035em]"><?php esc_html_e( 'Nothing found', 'probo-connect-theme' ); ?></h1>
		<p class="mt-3 max-w-[560px] text-[17px] leading-relaxed text-ink-2">
			<?php esc_html_e( 'Try a different search term, or browse the full range.', 'probo-connect-theme' ); ?>
		</p>
